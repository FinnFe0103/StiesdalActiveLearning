...
hyperparameter_spaces = {
    'BNN': {
        ...
        'acquisition_function': ['US', 'UCB', 'PI', 'EI', 'RS'],
        'lambda_reg': [0.05, 0.01, 0.001],
        },
    'GP': {
        'scaling': ['Minmax', 'Standard'],
        'learning_rate': [0.01, 0.1],
...
model_performance_summary = [] # List to collect all model results
for model_name, params_space in hyperparameter_spaces.items():
    ...
    model_results = []  # List to store results of all runs for the current model
    ...
    for combination in random_combinations:
        run_name = f"{model_name}_{'_'.join([f'{k}_{v}' for k, v in combination.items()])}"
        model = RunModel(model_name=model_name, run_name=run_name, steps=steps, epochs=epochs, **combination)
        model_instances[model_name].append(model)

        aggregated_results = {'Model': run_name}
        #aggregated_results.update({f'{key}': value for key, value in combination.items()})  # Add hyperparameter settings
        ...
        for step in range(model.steps):
            ...
            print(f'---Training time: {time.time() - train_model_time:.2f} seconds')
            ...
            print(f'---Final prediction time: {time.time() - final_prediction_time:.2f} seconds')

            evaluate_pool_data_time = time.time()
            model.evaluate_pool_data(step) # Evaluate the model on the pool data
            print(f'---Evaluation on pool data time: {time.time() - evaluate_pool_data_time:.2f} seconds')
            ...
            print(f'---Acquisition function time: {time.time() - acquisition_function_time:.2f} seconds')
            ...
            print(f'---KL divergence time: {time.time() - get_kl_divergence_time:.2f} seconds')
            ...
            print(f'Updated pool and known data (AL = {model.acquisition_function}):', model.data_pool.shape, model.data_known.shape)
...
exclude_hyperparameters = ['verbose', 'writer', 'data', 'data_known', 'data_pool', 'mse', 'mae', 'device', 'y_total_actual', 'pdf_p', 'validation_size'] # Paramters not included in the output
...
excel_filename = 'tested_hyperparameters.xlsx'
df_hyperparameters.to_excel(excel_filename, index=False)

print(f'Tested hyperparameters have been saved to {excel_filename}.')